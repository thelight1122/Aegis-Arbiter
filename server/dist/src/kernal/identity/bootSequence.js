...
export class AuthoritySeveranceService {
    ...
    `).get();
        const canonHash = createHash("sha256")
            .update(latestCanon?.content_json || "LOCKED_CANON_v1")
            .digest("hex");
    ...
    `).run(auditId, token.initialized_at, "Authority Severance complete. AEGIS substrate active.", JSON.stringify(token));
        return token;
    }
}
