...
export function ensureAegisSeed(db, params) {
    const { orgId, orgName, userId, displayName } = params;
    db.prepare(`INSERT OR IGNORE INTO aegis_orgs (id, name) VALUES (?, ?)`).run(orgId, orgName);
    db.prepare(`INSERT OR IGNORE INTO aegis_profiles (user_id, org_id, display_name) VALUES (?, ?, ?)`).run(userId, orgId, displayName ?? null);
    ...
    const ins = db.prepare(`INSERT OR IGNORE INTO aegis_marker_catalog (id, category, title, description)
     VALUES (?, 'domain', ?, ?)`);
    for (const r of roots)
        ins.run(r.id, r.title, r.description);
    ...
    db.prepare(`INSERT OR IGNORE INTO aegis_temporal_tensors
      (id, scope_type, scope_id, tensor_version, axioms, definitions, constraints)
     VALUES (?, 'org', ?, '1.0.0', ?, '{}', ?)`).run(uuid(), orgId, JSON.stringify(axioms), JSON.stringify(constraints));
}
//# sourceMappingURL=aegisSeed.js.map