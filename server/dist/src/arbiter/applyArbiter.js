...
export async function applyArbiterFlagOnly(args) {
    ...
    // ------------------------------------------------------------
    let aegisGate = { attempted: false, paused: false };
    try {
        ...
        if (!session) {
            ...
            const observed = violatedRootMarkerIds;
            aegisGate = {
                attempted: true,
        ...
        aegisGate = {
            attempted: false,
            paused: false,
            note: `AEGIS gate skipped due to error: ${err?.message ?? String(err)}`,
        };
    }
    // ------------------------------------------------------------
    ...
    if (flagged || aegisGate.paused) {
        ...
        const gateLine = aegisGate.attempted && typeof aegisGate.integrity_resonance === "number"
...
//# sourceMappingURL=applyArbiter.js.map