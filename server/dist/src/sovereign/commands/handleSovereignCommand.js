...
function nativeGet(native, sql, params = []) {
    if (!native || typeof native.prepare !== "function")
        return null;
    const stmt = native.prepare(sql);
    const row = stmt.get(...params);
    return row ?? null;
}
function nativeAll(native, sql, params = []) {
    if (!native || typeof native.prepare !== "function")
        return [];
    const stmt = native.prepare(sql);
...
function hasTable(native, tableName) {
    const row = nativeGet(native, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", [tableName]);
    return !!row?.name;
}
...
function chooseScope(aegisSession) {
    if (!aegisSession)
        return null;
    const orgId = aegisSession.org_id;
    const userId = aegisSession.user_id;
    ...
    switch (cmd.kind) {
        ...
        case "AEGIS_STATUS": {
            ...
            if (!native) {
                return {
                    ok: false,
                    message: "Status unavailable: native SQLite handle not found on SqliteDb (expected db.db).",
                };
            }
            ...
            if (!hasAegisSessions) {
                return {
                    ok: false,
                    message: "AEGIS tables not present (aegis_sessions missing). Apply migrations / run local bootstrap.",
                    payload: {
                        visibility: {
                            tables: {
            ...
            const aegisSession = nativeGet(native, `SELECT id, org_id, user_id, status, started_at, ended_at, integrity_resonance, peer_state
         FROM aegis_sessions
         WHERE id = ?`, [sessionId]);
            if (!aegisSession) {
                return {
                    ok: false,
                    message: "AEGIS session row not found. Seed/bootstrap may not have mirrored sessions into aegis_sessions yet.",
                    payload: {
                        sessionId,
                        baseSession,
                        visibility: {
            ...
            const markerEvents = hasMarkerEvents && orgId
                ? nativeAll(native, `SELECT id, event_type, scope_type, scope_id, marker_id, detail, created_at, session_id, user_id, org_id
               FROM aegis_marker_events
               WHERE org_id = ?
                 AND (session_id = ? OR (scope_type = ? AND scope_id = ?))
               ORDER BY created_at DESC, id DESC
               LIMIT ?`, [orgId, sessionId, scope?.scopeType ?? "org", scope?.scopeId ?? orgId, limitEvents])
                : [];
            // Candidates / learned are stored per scope, not per session.
            const candidates = hasCandidates && scope
                ? nativeAll(native, `SELECT id, scope_type, scope_id, marker_id, status,
               ...
               LIMIT ?`, [scope.scopeType, scope.scopeId, limitCandidates])
                : [];
            const learned = hasLearned && scope
                ? nativeAll(native, hasCatalog
                   ...
                   WHERE scope_type = ? AND scope_id = ?
                   ORDER BY last_seen_at DESC, id DESC
                   LIMIT ?`, [scope.scopeType, scope.scopeId, limitLearned])
                : [];
            const msgParts = [];
            msgParts.push("AEGIS status loaded.");
            msgParts.push(`sessionId=${sessionId}`);
            const baseStatus = baseSession ? baseSession.status : undefined;
            if (typeof baseStatus !== "undefined") {
                msgParts.push(`sessions.status=${String(baseStatus)}`);
            }
            msgParts.push(`aegis_sessions.status=${String(aegisSession.status)}`);
            msgParts.push(`integrity_resonance=${String(aegisSession.integrity_resonance)}`);
            if (orgId)
                msgParts.push(`orgId=${orgId}`);
            if (userId)
                msgParts.push(`userId=${userId}`);
            if (scope)
                msgParts.push(`scope=${scope.scopeType}:${scope.scopeId}`);
            msgParts.push(`events=${markerEvents.length}, candidates=${candidates.length}, learned=${learned.length}`);
            return {
                ok: true,
                ...
                payload: {
                    session: {
                        ...
                        aegis: {
                            ...aegisSession,
                            started_at: safeIso(aegisSession.started_at) ?? aegisSession.started_at,
                            ended_at: safeIso(aegisSession.ended_at) ?? aegisSession.ended_at,
                        },
                        scope,
                    },
                    visibility: {
                    ...
                    markerEvents: markerEvents.map((e) => ({
                        ...e,
                        created_at: safeIso(e.created_at) ?? e.created_at,
                    })),
                    candidates: candidates.map((c) => ({
                        ...c,
                        first_seen_at: safeIso(c.first_seen_at) ?? c.first_seen_at,
                        last_seen_at: safeIso(c.last_seen_at) ?? c.last_seen_at,
                        created_at: safeIso(c.created_at) ?? c.created_at,
                        updated_at: safeIso(c.updated_at) ?? c.updated_at,
                    })),
                    learned: learned.map((l) => ({
                        ...l,
                        first_learned_at: safeIso(l.first_learned_at) ?? l.first_learned_at,
                        last_seen_at: safeIso(l.last_seen_at) ?? l.last_seen_at,
                        created_at: safeIso(l.created_at) ?? l.created_at,
                        updated_at: safeIso(l.updated_at) ?? l.updated_at,
                    })),
                },
            };
        }
...
//# sourceMappingURL=handleSovereignCommand.js.map