...
export function parseSovereignCommand(input) {
    ...
    if (sub === "storage") {
        ...
        if (op === "status")
            return { kind: "STORAGE_STATUS" };
        if (op === "set") {
            const mode = (rest[2] ?? "minimal").toLowerCase();
            if (mode !== "minimal" && mode !== "standard" && mode !== "verbose")
                return { kind: "HELP" };
...
//# sourceMappingURL=parseSovereignCommand.js.map