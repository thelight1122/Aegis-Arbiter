...
export type Finding = {
  ...
  severity: 1 | 2 | 3 | 4 | 5;
  evidence: string;
  index: number; // character index in the analyzed text
};
...
type Flags = { rbc: boolean; arbiter: boolean; lint: boolean };
...
function emptyCounts(): Record<FindingType, number> {
...
function findAll(text: string, re: RegExp): Array<{ index: number; match: string }> {
  ...
  while ((m = rr.exec(text)) !== null) {
    ...
    if (m.index === rr.lastIndex) rr.lastIndex++;
  }
  return out;
}

export function analyzeText(text: string, flags: Flags): Analysis {
  ...
  if (flags.rbc) notes.push("RBC mode: tolerant, seeks de-escalation signals.");
  if (flags.arbiter) notes.push("Arbiter mode: strict, flags force/ultimatum fast.");
  if (flags.lint) notes.push("Lint mode: content/style warnings for drift + certainty.");

  // A tiny “sanity note” if nothing is detected.
  if (findings.length === 0) notes.push("No heuristic triggers detected.");
