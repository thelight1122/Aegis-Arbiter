// FILE: server/src/analyzeText.ts
// Server-safe analyzer entrypoint (NodeNext/ESM friendly).
const RULES = [
...
export function analyzeText(input) {
    ...
    for (const rule of RULES) {
        ...
        while ((m = rule.rx.exec(text)) !== null) {
            findings.push({ type: rule.type, severity: rule.severity, evidence: m[0], index: m.index });
